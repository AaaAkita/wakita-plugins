...
```
wakita-plugins
├── wakita-governance（管控核心 v2.0.5）    ← 行为约束层
│   ├── 危险操作拦截（PreToolUse）
│   ├── 审计留痕（PostToolUse）
...
- `/audit [行数]` - 查看最近审计日志
- `/lock <文件路径>` - 临时加锁保护文件
- `/submodel` - 交互式切换三个子智能体的运行模型

#### ⚙️ 安装后配置脚本

子智能体（scout/auditor/builder）的 `model:` 字段不支持环境变量展开，用户安装后若想换 provider/model，有**两种方式**：

**方式一：`/submodel` 命令（推荐，交互式）**

输入 `/submodel`，主智能体会读取本机 `~/.zcode/v2/config.json`，用 `AskUserQuestion` 列出所有可用 provider 和 model 供你选择，确认后自动注入。适合不熟悉 config.json 结构的用户。

**方式二：直接跑脚本（进阶，命令行）**

```bash
# 列出所有可用 provider/model
...
适合脚本化、CI 场景，或已知道 provider key 的用户。跨平台 Python，同时支持 config.json 中 `provider` 为 dict / list 两种结构。详见 `scripts/README.md`。

> ⚠️ **生效方式**：ZCode 当前无热重载/重置会话功能（关注官方更新），切换后需**关闭并重开当前会话**或**重启 ZCode 客户端**才能生效。
