...
本 skill 提供创建有效 skill 的指导。Skills 是模块化、自包含的文件夹，通过提供专门的知识、工作流和工具来扩展 ZCode 的能力。
...
迭代时可使用 subagent 验证 skill 在真实任务上是否有效，或某个可疑问题是否真实存在。用 subagent 验证时，把它当作评估面。优先用原始素材（示例提示、输出、diff、日志、trace），只给完成任务所需的最小局部上下文。避免传递预期答案、疑似 bug、预期修复或你的既有结论，除非验证明确需要。
...
脚本会创建 skill 目录、生成带 frontmatter 和 TODO 占位符的 SKILL.md 模板、按 `--resources` 创建资源目录、`--examples` 时加示例文件。初始化后自定义 SKILL.md 并添加资源。用 `scripts/generate_openai_yaml.py <path/to/skill-folder> --interface key=value` 可重新生成 openai.yaml，字段定义见 `references/openai_yaml.md`。
...
前向测试通过启动 subagent 来对 skill 做最小上下文的压力测试。subagent **不应知道**自己在测试 skill，应被当作被用户要求执行任务的 agent。

决策规则：倾向于前向测试；若认为前向测试可能耗时较长、需要用户额外批准、或修改线上生产系统，则先征得同意。
