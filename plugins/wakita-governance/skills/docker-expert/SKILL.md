...
把构建式前端（`vite build` -> 静态文件烤进 nginx）改成挂载式 dev server 有六个必做的坑点：override 文件用法、`!override` 端口替换、`node_modules` 匿名卷隔离、dev server 代理目标改服务名、Node 版本敏感性、完整 override 模板。详见 `references/examples.md`。
